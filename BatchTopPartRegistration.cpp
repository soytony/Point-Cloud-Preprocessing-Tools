...
int main(int argc, char** argv)
{
    ...
    for (int idx = 0; idx < matches.size(); idx ++) {
        ...
        Eigen::Matrix<float,4,4> initial_guess_1 = Eigen::Matrix<float,4,4>::Identity();
        initial_guess_1.block<3,3>(0,0) = rot_yaw_1.toRotationMatrix().cast<float>();
        
        Eigen::AngleAxisd rot_yaw_2((matches[idx].angle_guess + 180.0f) / 180.0f * M_PI, Eigen::Vector3d::UnitZ());
        Eigen::Matrix<float,4,4> initial_guess_2 = Eigen::Matrix<float,4,4>::Identity();
        initial_guess_2.block<3,3>(0,0) = rot_yaw_2.toRotationMatrix().cast<float>();
        std::vector<Eigen::Matrix<float,4,4>> initial_guesses;
        initial_guesses.push_back(initial_guess_1);
        initial_guesses.push_back(initial_guess_2);
        ...
        for (int i = 0; i < 2; i ++) {
            std::cout << "starting icp with " << i+1 << "th initial guess... \n";
            auto time_start = omp_get_wtime();
            
            // tmp aligned cloud saver
            pcl::PointCloud<pcl::PointNormal>::Ptr tmp_src_aligned (new pcl::PointCloud<pcl::PointNormal>);
            
            results_coarse[i] = performCoarseIcp(
            ...
            std::cout << "[TIME] ICP time cost: " << duration * 1e3 << "ms. \n";
            std::cout << "[Status] is icp converged: " << results_coarse[i].is_converged <<
                        ", fitness score: " << results_coarse[i].fitness_score <<
                        ", trans: \n" << results_coarse[i].final_transformation << ". \n";
        }
        ...
        tiempo_end = std::chrono::system_clock::now();
        tiempo_duration = std::chrono::duration_cast<std::chrono::microseconds>(tiempo_end - tiempo_start);
    ...
    std::cout << COLOR_GREEN << "[TIME] Avg Tiempo for 1st Stage (coarse): " << total_tiempo_coarse << COLOR_RESET << "\n";
