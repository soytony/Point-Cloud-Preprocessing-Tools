...
int main(int argc, char** argv)
{
    ...
    std::ofstream precison_report("./icp_precision_report.txt");
    ...
    for (int idx = 0; idx < matches.size(); idx ++) {
        ...
        flat_cloud_1.reset(new pcl::PointCloud<pcl::PointXYZ>);
        flat_cloud_2.reset(new pcl::PointCloud<pcl::PointXYZ>);
        flat_cloud_1_ds.reset(new pcl::PointCloud<pcl::PointXYZ>);
        flat_cloud_2_ds.reset(new pcl::PointCloud<pcl::PointXYZ>);
        flat_cloud_1_ds_aligned.reset(new pcl::PointCloud<pcl::PointXYZ>);
        points_with_normals_src.reset(new pcl::PointCloud<pcl::PointNormal>);
        points_with_normals_tgt.reset(new pcl::PointCloud<pcl::PointNormal>);
        points_with_normals_src_aligned.reset(new pcl::PointCloud<pcl::PointNormal>);
        ...
        std::cout << "[TIME] Extract top and flaten: " << duration.count() * 1e-3 << "ms. \n" << endl;
        ...
        std::cout << "[TIME] Down sampling: " << duration.count() * 1e-3 << "ms. \n" << endl;


        time_start = std::chrono::system_clock::now();
        Eigen::AngleAxisd rot_yaw_1(matches[idx].angle_guess / 180.0f * M_PI, Eigen::Vector3d::UnitZ());
        Eigen::Matrix<double,4,4> initial_guess_1 = Eigen::Matrix<double,4,4>::Identity();
        initial_guess_1.block<3,3>(0,0) = rot_yaw_1.toRotationMatrix();
        
        Eigen::AngleAxisd rot_yaw_2((matches[idx].angle_guess + 180.0f) / 180.0f * M_PI, Eigen::Vector3d::UnitZ());
        Eigen::Matrix<double,4,4> initial_guess_2 = Eigen::Matrix<double,4,4>::Identity();
        initial_guess_2.block<3,3>(0,0) = rot_yaw_2.toRotationMatrix();


        addNormal(flat_cloud_1_ds, points_with_normals_src);
        addNormal(flat_cloud_2_ds, points_with_normals_tgt);
        time_end = std::chrono::system_clock::now();
        duration = std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_start);
        std::cout << "[TIME] computing normals: " << duration.count() * 1e-3 << "ms. \n" << endl;
        
        std::vector<IcpAlignResult> results_coarse(2);

        auto time_omp_start = omp_get_wtime();

        std::cout << "starting icp with 1st initial guess... \n";
        time_start = std::chrono::system_clock::now();
        results_coarse[0] = performCoarseIcp(
                points_with_normals_src,
                points_with_normals_tgt,
                points_with_normals_src_aligned,
                initial_guess_1.cast<float>());
        time_end = std::chrono::system_clock::now();
        duration = std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_start);
        std::cout << "[TIME] ICP 1 time cost: " << duration.count() * 1e-3 << "ms. " << endl;
        std::cout << "is icp converged: " << results_coarse[0].is_converged <<
                    ", fitness score: " << results_coarse[0].fitness_score <<
                    ", trans: \n" << results_coarse[0].final_transformation << ". \n";

        std::cout << "starting icp with 2nd initial guess... \n";
        time_start = std::chrono::system_clock::now();
        results_coarse[1] = performCoarseIcp(
                points_with_normals_src,
                points_with_normals_tgt,
                points_with_normals_src_aligned,
                initial_guess_2.cast<float>());
        time_end = std::chrono::system_clock::now();
        duration = std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_start);
        std::cout << "[TIME] ICP 2 time cost: " << duration.count() * 1e-3 << "ms. \n" << endl;
        std::cout << "is icp converged: " << results_coarse[1].is_converged <<
                    ", fitness score: " << results_coarse[1].fitness_score <<
                    ", trans: \n" << results_coarse[1].final_transformation << ". \n";

        auto time_omp_end = omp_get_wtime();
        auto omp_duration = time_omp_end - time_omp_start;
        std::cout << "[TIME] two icps: " << omp_duration * 1e3 << "ms. \n" << endl;


        auto best_result_coarse = (results_coarse[0].fitness_score < results_coarse[1].fitness_score) ?
                results_coarse[0] : results_coarse[1];
        std::cout << "best coarse result: \n" <<
                    "fitness score: " << best_result_coarse.fitness_score << "\n"
                    "trans: \n" << best_result_coarse.final_transformation << ". \n";
        ...
        total_tiempo_coarse += tiempo_duration.count() * 1e-3;

        if (!is_use_refinement) {
            continue;
        }
        ...
        if (result_fine.fitness_score > 4.0f) {
            std::cout << COLOR_RED << "3D ICP Failed. " << COLOR_RESET << "\n";
        } else {
            float diff_x = result_fine.final_transformation(0,3) - best_result_coarse.final_transformation(0,3);
            float diff_y = result_fine.final_transformation(1,3) - best_result_coarse.final_transformation(1,3);
            float diff_xy = std::sqrt(diff_x * diff_x + diff_y * diff_y);
            
            auto rela_rot = result_fine.final_transformation.block<3,3>(0,0).inverse() * best_result_coarse.final_transformation.block<3,3>(0,0);
            // Eigen::Vector3f fine_angles = result_fine.final_transformation.block<3,3>(0,0).eulerAngles(2, 1, 0); 
            // Eigen::Vector3f coarse_angles = best_result_coarse.final_transformation.block<3,3>(0,0).eulerAngles(2, 1, 0); 
            // Eigen::Vector3f diff_angles = rela_rot.eulerAngles(2,1,0);
            Eigen::Vector3f diff_angles = rotationMatrixToEulerAngles(rela_rot);
            // float diff_yaw = (fine_angles(0) - coarse_angles(0)) / M_PI * 180.0f;
            float diff_yaw = diff_angles(2) / M_PI * 180.0f;
            if (diff_yaw > 180.0f) {diff_yaw -= 360.0f;}
            if (diff_yaw <-180.0f) {diff_yaw += 360.0f;}

            std::cout << "diff_xy: " << diff_xy << ", diff_yaw: " << diff_yaw << "\n";
            precison_report << diff_xy << " " << diff_yaw << "\n";
        }

    }

    total_tiempo_coarse /= matches.size();
    std::cout << COLOR_GREEN << "avg tiempo for first stage: " << total_tiempo_coarse << COLOR_RESET << "\n";
