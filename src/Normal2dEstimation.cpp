...
void Normal2dEstimation::compute(const pcl::PointCloud<pcl::Normal>::Ptr& normal_cloud) const {
    ...
    if (m_in_cloud->is_dense) {
        ...
        for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
            if(this->searchForNeighbors((*m_indices)[idx], *nn_indices, nn_dists) ==0 ) {
                failed_count ++;
                normal_cloud->points[idx].normal_x = normal_cloud->points[idx].normal_y = normal_cloud->points[idx].normal_z = 0.0;
            ...
            // std::cout << "indices size: " << nn_indices->size() << "\n";
            // std::cout << normal_cloud->points[idx].normal_x << " " << normal_cloud->points[idx].normal_y << " " << normal_cloud->points[idx].normal_z << "\n";
    ...
    // std::cout << "failed count:" << failed_count << ", success count: " << success_count << ".\n";
...
bool Normal2dEstimation::computePointNormal2d (boost::shared_ptr<std::vector<int>>& indices,
    ...
    if (indices->size()==2){
        ...
        vect_x = m_in_cloud->points[(*indices)[0]].x - m_in_cloud->points[(*indices)[1]].x;
        vect_y = m_in_cloud->points[(*indices)[0]].y - m_in_cloud->points[(*indices)[1]].y;
        norm = std::sqrt(vect_x * vect_x + vect_y * vect_y);
        vect_x /= norm;
        vect_y /= norm;
        nx = -vect_y;
        ny = vect_x;
    ...
    pca.setInputCloud(m_in_cloud);
    pca.setIndices(indices);
    auto eigen_vec = pca.getEigenVectors();
    auto result = eigen_vec.col(1);
    // std::cout << "eigen vec:" << eigen_vec << "\n";
    nx = eigen_vec(0,1);
    ny = eigen_vec(1,1);
    nz = 0.0;
    curvature = eigen_vec(1) /  (eigen_vec(0)  + eigen_vec(1) );
    return true;
}
